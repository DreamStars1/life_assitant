...
在现有 `lifeassistant-server` 模块中嵌入 MCP Server，通过 **SSE（Server-Sent Events）** 对外暴露。服务端部署，客户端远程连接。
...
所有工具方法均通过 Spring AI 的 `org.springaicommunity.mcp.annotation.@McpTool` 注解声明。Token 从 HTTP Header 提取，工具参数中不出现。
...
在父 POM 的 `<dependencyManagement>` 中引入 Spring AI BOM：
...
当前项目使用 ContiNew Starter 2.15.0 → Spring Boot 3.4.10，Spring AI 1.1.x 兼容 Spring Boot 3.4.x。`spring-ai-starter-mcp-server-webmvc` 使用 SSE 传输，默认端点路径为 `/sse`。

已知约束：工具类使用 `org.springaicommunity.mcp.annotation.@McpTool` / `@McpToolParam` 注解（来自 `mcp-annotations:0.9.0` 社区包），不继承 Spring Web 的注解语义。工具类内部直接注入 Spring Service bean。

### 客户端配置参考

Spring AI 1.1.x SSE 传输使用两个端点：
- 建立 SSE 连接：`GET /sse`
- 发送 JSON-RPC 消息：`POST /mcp/message`

后端端口：开发环境 `8000`，生产环境 `443`（HTTPS）。

**重要：** 先通过前端设置页创建 API Token，再将 `la_xxx` 替换为真实 token。
...
| 客户端 | 配置 |
|--------|------|
| **MCP Inspector**<br>（官方 GUI 测试工具） | `npx @modelcontextprotocol/inspector http://localhost:8000/sse`<br><br>浏览器打开后，在 Inspector 的 Headers 中添加 `Authorization: Bearer la_xxx`，点击 Connect。 |
| **curl**<br>（快速验证连接） | `curl -N -H "Authorization: Bearer la_xxx" http://localhost:8000/sse`<br><br>看到 `event: endpoint` 和 `data: /mcp/message` 即连接成功。 |
| **Claude Desktop**<br>（via mcp-remote） | `claude_desktop_config.json`：<br>```json{  "mcpServers": {    "life-assistant": {      "command": "npx",      "args": [        "mcp-remote@latest",        "--sse",        "http://localhost:8000",        "--header",        "Authorization: Bearer la_xxx"      ]    }  }}``` |
| **Cursor** | `.cursor/mcp.json`：<br>```json{  "mcpServers": {    "life-assistant": {      "type": "sse",      "url": "http://localhost:8000/sse",      "headers": {        "Authorization": "Bearer la_xxx"      }    }  }}``` |
| **Workbuddy**<br>（CodeBuddy） | ```json{  "mcpServers": {    "life-assistant": {      "type": "sse",      "url": "http://localhost:8000/sse",      "headers": {        "Authorization": "Bearer la_xxx"      }    }  }}``` |
| **Claude Code** | `claude mcp add life-assistant --transport sse http://localhost:8000/sse --header "Authorization: Bearer la_xxx"` |

---

#### 生产环境

部署域名 `https://api.life-assitant.top`，后端通过 Nginx 反代到 `:8000`。

| 客户端 | 配置 |
|--------|------|
| **Claude Desktop**<br>（via mcp-remote） | `claude_desktop_config.json`：<br>```json{  "mcpServers": {    "life-assistant": {      "command": "npx",      "args": [        "mcp-remote@latest",        "--sse",        "https://api.life-assitant.top",        "--header",        "Authorization: Bearer la_xxx"      ]    }  }}``` |
| **Cursor** | `.cursor/mcp.json`：<br>```json{  "mcpServers": {    "life-assistant": {      "type": "sse",      "url": "https://api.life-assitant.top/sse",      "headers": {        "Authorization": "Bearer la_xxx"      }    }  }}``` |
| **Workbuddy**<br>（CodeBuddy） | ```json{  "mcpServers": {    "life-assistant": {      "type": "sse",      "url": "https://api.life-assitant.top/sse",      "headers": {        "Authorization": "Bearer la_xxx"      }    }  }}``` |
| **Claude Code** | `claude mcp add life-assistant --transport sse https://api.life-assitant.top/sse --header "Authorization: Bearer la_xxx"` |
