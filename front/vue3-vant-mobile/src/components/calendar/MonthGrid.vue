...
<style scoped>
.month-grid { width: 100%; user-select: none; }
.weekday-header { display: grid; grid-template-columns: repeat(7, 1fr); text-align: center; font-weight: 600; padding: 8px 0; color: var(--van-text-color-2); font-size: 13px; }
.days-grid { display: grid; grid-template-columns: repeat(7, 1fr); gap: 2px; }
.day-cell { padding: 4px; min-height: 48px; border-radius: 8px; background: var(--van-cell-background); cursor: pointer; transition: background 0.2s; }
.day-cell:hover { background: #FFF5EE; }
.day-cell.other-month { opacity: 0.3; }
.day-cell.is-today { background: #FFF0E6; }
.day-cell.is-today .day-num { color: #E8905E; font-weight: 700; }
.day-num { font-size: 13px; color: var(--van-text-color); font-weight: 500; }
.event-dots { display: flex; flex-wrap: wrap; gap: 3px; margin-top: 2px; }
.event-dot { width: 6px; height: 6px; border-radius: 50%; }
.more { font-size: 10px; color: var(--van-text-color-3); }
.log-icons { display: flex; flex-wrap: wrap; gap: 2px; margin-top: 2px; font-size: 14px; }
